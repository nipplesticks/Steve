...
)
{
  ...
  if (light.type == POINT_LIGHT)
  {
    ...
    {
      float attenuation = light.intensity / (1.0f + light.dropOff * pow(distanceToLight, light.pow));
      halfWayDir = normalize(normalize(positionToLight) + worldToCamDir);
      //halfWayDir = normalize(positionToLight + worldToCamDir);
      specularHighlight += pow(max(dot(pixelWorldNormal, halfWayDir), 0.0f), roughness) * pixelMetallic.r * attenuation * light.color;
      finalColor = max(dot(pixelWorldNormal, normalize(positionToLight)), 0.0f) * light.color * pixelAlbedo * attenuation;
    }
  }
  else if (light.type == DIRECTIONAL_LIGHT)
  {
    halfWayDir = normalize(normalize(-light.positionOrDirection) + worldToCamDir);
    specularHighlight += pow(max(dot(pixelWorldNormal, halfWayDir), 0.0f), roughness) * pixelMetallic.r * light.intensity * light.color;
    finalColor = max(dot(pixelWorldNormal, normalize(-light.positionOrDirection)), 0.0f) * light.color * pixelAlbedo * light.intensity;
  }
