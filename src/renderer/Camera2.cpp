...
void Camera2::SetPosition(float x, float y, float z, bool calcAxis)
{
  SetPosition(DM::Vec3f(x, y, z), calcAxis);
}

void Camera2::SetPosition(const DM::Vec3f& position, bool calcAxis)
{
  myPosition = position;
  if (calcAxis)
    _calcAxis();
...
void Camera2::SetLookAt(float x, float y, float z, bool calcAxis)
{
  SetLookAt(DM::Vec3f(x, y, z), calcAxis);
}

void Camera2::SetLookAt(const DM::Vec3f& lookAt, bool calcAxis)
{
  myLookAt = lookAt;
  if (calcAxis)
    _calcAxis();
...
void Camera2::Roll(float d)
{
  DirectX::XMVECTOR rot =
      DirectX::XMQuaternionRotationAxis((myPosition - myLookAt).Normalize().Load(), d);
